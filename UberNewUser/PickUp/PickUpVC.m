...
@interface PickUpVC ()
{
    ...
    GMSMapView *mapView_;
    BOOL is_paymetCard,is_Fare,is_set_address,is_destination,is_paid;
    int flag;
...
- (void)viewDidLoad
{
	...
	[self.bReferralSkip setBackgroundColor:LightBtnColor];
	[self.bReferralSubmit setBackgroundColor:LightBtnColor];
	[self.btnGetFare setBackgroundColor:DarkBtnColor];
	[self.btnOK setBackgroundColor:DarkBtnColor];
	[self.btnRidenow setBackgroundColor:LightBtnColor];
	[self.btnRidelater setBackgroundColor:LightBtnColor];
	[self.btnCash setBackgroundColor:DarkBtnColor];
	[self.btnCard setBackgroundColor:LightBtnColor];
	[self.lblServiceType setBackgroundColor:ProfileViewColor];
	[self.lblServiceType setTextColor:ServiceTypeText];
	[self.btnShedule setBackgroundColor:LightBtnColor];
	//[self.imgAlertBg setBackgroundColor:[UIColor redColor]];
	self.imgAlertBg.layer.cornerRadius = self.imgAlertBg.frame.size.width / 2;
...
-(void)getPlaces
{
    ...
    NSString *url=[NSString stringWithFormat:@"https://maps.googleapis.com/maps/api/place/nearbysearch/json?sensor=true&key=%@&location=%f,%f&radius=500",strForGoogleMapKey,[strForLatitude floatValue],[strForLongitude floatValue]];
...
- (void)customSetup
{
    ...
    if ( revealViewController )
    {
        ...
        [self.btnMenu addTarget:self action:@selector(onClickMenu) forControlEvents:UIControlEventTouchUpInside];
...
- (void)onClickMenu
{
    ...
    [self.viewForPreferral endEditing:YES];
    [self.viewForFareAddress endEditing:YES];
}

-(void)SetLocalization
{
    self.lblContactingNearByDriver.text = NSLocalizedString(@"REQUESTING DRIVER", nil);
    ...
    self.lblMakeRequest.text = NSLocalizedString(@"MAKE_REQUEST", nil);
    self.lblEstTime.text = NSLocalizedString(@"EST_TIME", nil);
    self.lblBasePrice.text = NSLocalizedString(@"Base Price :", nil);
    self.lblDistanceCost.text = NSLocalizedString(@"Distance Cost", nil);
    self.lblMaxSize.text = NSLocalizedString(@"Max Size :", nil);
    self.lblETATitle.text = NSLocalizedString(@"ETA", nil);
    self.lblMinFare.text = NSLocalizedString(@"MIN FARE", nil);
    self.lblEstimateTimeMessage.text = NSLocalizedString(@"Estimate Time Msg", nil);
    
    self.lblEstiMin.text = NSLocalizedString(@"Time Cost :", nil);
    self.lblTimeCost.text = NSLocalizedString(@"TIME COST", nil);
    self.txtDestination.placeholder = NSLocalizedString(@"Destination Address", nil);
    [self.btnCash setTitle:NSLocalizedString(@"Cash", nil) forState:UIControlStateNormal];
    [self.btnCash setTitle:NSLocalizedString(@"Cash", nil) forState:UIControlStateSelected];
    [self.btnCard setTitle:NSLocalizedString(@"Card", nil) forState:UIControlStateNormal];
    [self.btnCard setTitle:NSLocalizedString(@"Card", nil) forState:UIControlStateSelected];
    self.txtPromoCode.placeholder = NSLocalizedString(@"Enter Promo Code", nil);
    [self.btnRidenow setTitle:NSLocalizedString(@"Ride Now", nil) forState:UIControlStateNormal];
    [self.btnRidenow setTitle:NSLocalizedString(@"Ride Now", nil) forState:UIControlStateSelected];
    [self.btnRidelater setTitle:NSLocalizedString(@"Ride Later", nil) forState:UIControlStateNormal];
    [self.btnRidelater setTitle:NSLocalizedString(@"Ride Later", nil) forState:UIControlStateSelected];
}
...
- (IBAction)ETABtnPressed:(id)sender
{
    ...
    [self.btnGetFare setTitle:NSLocalizedString(@"GET FARE ESTIMATE", nil) forState:UIControlStateNormal];
}
...
-(void)CardButtonMethos
{
	...
	[self.btnCash setBackgroundColor:LightBtnColor];
	[self.btnCard setBackgroundColor:DarkBtnColor];
    ...
    if([CLLocationManager locationServicesEnabled])
    {
        ...
        if(![strForTypeid isEqualToString:@"0"])
        {
            ...
            else
            {
                ...
                {
                    [[AppDelegate sharedAppDelegate]showLoadingWithTitle:NSLocalizedString(@"REQUESTING DRIVER", nil)];
                    ...
                    [afn getDataFromPath:FILE_CREATE_FUTURE_REQUEST withParamData:dictParam withBlock:^(id response, NSError *error)
                     {
                         ...
                         if (response)
                         {
                             ...
                             if([[response valueForKey:@"success"]intValue]==1)
                             {
                                 ...
                                 UIAlertView *alert=[[UIAlertView alloc] initWithTitle:nil message:@"Thank You for your Booking. Your driver details will be shared 30 mins before your pickup time" delegate:self cancelButtonTitle:@"Cancel" otherButtonTitles:@"Ok", nil];
                                 alert.tag=300;
                                 [alert show];
                             }
                             else
...
- (IBAction)pickMeUpBtnPressed:(id)sender
{
    ...
    [UIView animateWithDuration:0.5 animations:^{
        ...
        client_marker.position = coor;
        client_marker.icon = [UIImage imageNamed:@"pin_client_org"];
        client_marker.map = mapView_;
        [_txtPickupAddress setEnabled:NO];
        [_viewForMarker setHidden:YES];
...
-(void)HideRequestView
{
    [UIView animateWithDuration:0.5 animations:^{
        ...
        self.btnViewClose.hidden=YES;
        self.tableForCity.hidden=YES;
        [_viewForMarker setHidden:NO];
        [_txtPickupAddress setEnabled:YES];
        [mapView_ clear];
...
//                     bill=[response valueForKey:@"bill"];
//                    
                     if(strCheck)
                     {
                         self.btnCancel.hidden=YES;
                 ...
                 else if (![[response valueForKey:@"success"]boolValue])
                 {
                     if ([[response valueForKey:@"error_code"] integerValue]==406)
                     {
                         [self performSegueWithIdentifier:@"logout" sender:self];
...
-(void)getMinFareWithDistance:(NSString *)dist andETA:(NSString*)eTime
{
    strForUserId=[PREF objectForKey:PREF_USER_ID];
    strForUserToken=[PREF objectForKey:PREF_USER_TOKEN];
    ...
    [dictParam setValue:strForUserToken forKey:PARAM_TOKEN];
    [dictParam setValue:strForTypeid forKey:PARAM_TYPE];
    [dictParam setValue:dist forKey:PARAM_DISTANCE];
    [dictParam setValue:eTime forKey:@"time"];
    
    //[APPDELEGATE showLoadingWithTitle:NSLocalizedString(@"PLEASE_WAIT", nil)];
    if([[AppDelegate sharedAppDelegate]connected])
    {
        AFNHelper *afn=[[AFNHelper alloc]initWithRequestMethod:POST_METHOD];
        [afn getDataFromPath:FILE_GET_ESTIMATE_FARE withParamData:dictParam withBlock:^(id response, NSError *error)
         {
             NSLog(@"Get fare= %@",response);
             [APPDELEGATE hideLoadingView];
             if (response)
             {
                 if([[response valueForKey:@"success"] boolValue])
...
-(NSArray*) calculateRoutesFrom:(CLLocationCoordinate2D) f to: (CLLocationCoordinate2D) t
{
    ...
    else
    {
        ...
        NSArray *distance=[strDistance  componentsSeparatedByString:@" "];
        NSString *strDistanceValue = [distance objectAtIndex:0];
        //NSString *strDistanceUnit = [distance objectAtIndex:1];
        
        NSString *strTime = [[[getAddress objectAtIndex:0]objectAtIndex:0] valueForKey:@"text"];
        NSArray *time=[strTime  componentsSeparatedByString:@" "];
        NSString *strTimeValue = [time objectAtIndex:0];
        ...
        [self getMinFareWithDistance:strDistanceValue andETA:strTimeValue];
        self.tableforCityFare.hidden=YES;
        self.getFareView.hidden=YES;
...
-(void)getLocationFromString:(NSString *)str
{
    ...
    [dictParam setObject:strForGoogleMapKey forKey:PARAM_KEY];
         ...
         if(response)
         {
             ...
             if ([arrAddress count] > 0)
             {
                 if (flag==1)
                 {
                     ...
                     CLLocationCoordinate2D scoor=CLLocationCoordinate2DMake([strForLatitude doubleValue], [strForLongitude doubleValue]);
                     CLLocationCoordinate2D dcoor= CLLocationCoordinate2DMake([strForDestLatitude doubleValue], [strForDestLongitude doubleValue]);
                     [self.btnGetFare setTitle:self.txtDestiSearch.text forState:UIControlStateNormal];
...
- (UIView *)carousel:(iCarousel *)carousel viewForItemAtIndex:(NSInteger)index reusingView:(UIView *)view
{
    if(arrForApplicationType.count>0)
    {
        ...
        if (view == nil)
        {
            ...
            view = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 70.0f, 70.0f)];
