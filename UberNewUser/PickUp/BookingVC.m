...
- (void)viewDidLoad
{
    ...
    [super setNavBarTitle:NSLocalizedString(@"Booking Title", nil)];
    [self.btnBack setTitle:NSLocalizedString(@"Booking Title", nil) forState:UIControlStateNormal];
        ...
        [afn getDataFromPath:FILE_GETALLREQUESTS withParamData:dictParam withBlock:^(id response, NSError *error)
        {
            if(response)
            {
                if([[response valueForKey:@"success"] boolValue])
                {
                    ...
                    if(arrFuture.count!=0 || arrPresent.count!=0)
                    {
                    ...
                    {
                        [APPDELEGATE showToastMessage:NSLocalizedString(@"NO_BOOKINGS", nil)];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if([[dictData objectForKey:@"type"]isEqualToString:@"future"])
    {
        ...
        cell.lblTitle.text = NSLocalizedString(@"Trip yet start", nil);
    ...
    else if([[dictData objectForKey:@"type"]isEqualToString:@"progress"])
    {
        ...
        cell.lblTitle.text = NSLocalizedString(@"Trip Progress", nil);
