...
//  UberNewUser
//
//  Created by Elluminati on 29/10/14.
...
- (void)viewDidLoad
{
	...
	[self.btnPromoCancel setBackgroundColor:DarkBtnColor];
	[self.btnOK setBackgroundColor:LightBtnColor];
	[self.btnPromoApply setBackgroundColor:LightBtnColor];
...
-(void)SetLocalization
{
    ...
    self.lblTellUsWhy.text = NSLocalizedString(@"Tell us why", nil);
    self.lblDriverDelay.text = NSLocalizedString(@"Driver delayed", nil);
    self.lblChangedMind.text = NSLocalizedString(@"Changed mind", nil);
    self.lblBookAnotherCab.text = NSLocalizedString(@"Book another cab", nil);
    [self.btnPromoCode setTitle:NSLocalizedString(@"Enter Promo Code", nil) forState:UIControlStateNormal];
    [self.btnPromoCode setTitle:NSLocalizedString(@"Enter Promo Code", nil) forState:UIControlStateSelected];
    self.lblCarID.text = NSLocalizedString(@"Car ID", nil);
    self.lblPlateNumber.text = NSLocalizedString(@"Plate Number", nil);
}
        ...
        [afn getDataFromPath:strForUrl withParamData:nil withBlock:^(id response, NSError *error)
         {
             ...
             if (response)
             {
                 if([[response valueForKey:@"success"]boolValue])
                 {
                     ...
                     if([[response valueForKey:@"promo_code"] isEqualToString:@""])
                     {
                         self.btnPromoCode.enabled=YES;
                     }
                     else
                     {
                         [self.btnPromoCode setTitle:[response valueForKey:@"promo_code"] forState:UIControlStateNormal];
                         self.btnPromoCode.enabled=NO;
                     }
...
- (IBAction)onClickCloseCancelRequestView:(id)sender
{
    self.viewForCancelReason.hidden=YES;
}
            ...
            [afn getDataFromPath:FILE_APPLY_PROMO withParamData:dictParam withBlock:^(id response, NSError *error)
             {
                 ...
                 if (response)
                 {
                     if([[response valueForKey:@"success"]boolValue])
                     {
                         ...
                         NSString *errorCode =[NSString stringWithFormat:@"%@",[response valueForKey:@"error"]];
                         self.lblPromoMsg.text=[NSString stringWithFormat:@"%@",NSLocalizedString(errorCode, nil)];
                     ...
                     else
                     {
                         ...
                         {
                             NSString *errorCode =[NSString stringWithFormat:@"%@",[response valueForKey:@"error"]];
                             NSString *str=[NSString stringWithFormat:@"%@",NSLocalizedString(errorCode, nil)];
                             self.viewForPromoMessage.hidden=NO;
