...
//  UberforXOwner
//
//  Created by Elluminati on 22/11/14.
...
- (void)viewDidLoad
{
	...
	[self.btnContinue setBackgroundColor:LightBtnColor];
...
{
    Referral=@"0";
    if(self.txtCode.text.length < 1 || [self.txtCode.text isEqualToString:@" "])
        [APPDELEGATE showToastMessage:NSLocalizedString(@"Enter Referral Code",nil)];
    else
        [self createService];
}
