...
-(void)loginInFacebook:(LoginCompletionBlock)isLogin
{
    if (self.session.isOpen) {
        ...
        FBAccessTokenData *accessData=[FBAccessTokenData createTokenFromDictionary:[PREF objectForKey:UD_FBACCESSTOKENDATA]];
        ...
        if (accessData==nil) {
            ...
            [self.session openWithCompletionHandler:^(FBSession *session,
                ...
                [PREF setObject:[self.session.accessTokenData dictionary] forKey:UD_FBACCESSTOKENDATA];
                [PREF synchronize];
            ...
            [self.session openFromAccessTokenData:accessData completionHandler:^(FBSession *session, FBSessionState status, NSError *error) {
                ...
                if (self.session.isOpen) {
                    ...
                    [self.session openWithCompletionHandler:^(FBSession *session,
                        ...
                        [PREF setObject:[self.session.accessTokenData dictionary] forKey:@"FBAccessTokenDataDictionary"];
                        [PREF synchronize];
...
-(void)logOutFromFacebook
{
    ...
    [PREF removeObjectForKey:UD_FBACCESSTOKENDATA];
    [PREF synchronize];
