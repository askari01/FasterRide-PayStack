...
- (void)viewDidLoad
{
    ...
    [self.btnCancelRequest setTitle:NSLocalizedString(@"Cancel Request", nil) forState:UIControlStateSelected];
...
-(void)setTripDetails
{
    if([type isEqualToString:@"history"])
    {
        ...
        [self.btnNavigationTitle setTitle:NSLocalizedString(@"History", nil) forState:UIControlStateNormal];
    ...
    else if([type isEqualToString:@"future"])
    {
       // [self.img3 applyRoundedCornersFull];
        ...
        [self.btnNavigationTitle setTitle:NSLocalizedString(@"Future Request",nil) forState:UIControlStateNormal];
        [txtSource setText:[NSString stringWithFormat:@"%@",[self.dictInfo objectForKey:@"src_address"]]];
        [txtDestination setText:[NSString stringWithFormat:@"%@",[self.dictInfo objectForKey:@"dest_address"]]];
