...
- (void)viewDidLoad
{
	...
	[self.viewForPhoneNumber setHidden:NO];
...
-(void)SetLocalization
{
    ...
    self.lblPassword.text = NSLocalizedString(@"CURRENT PASSWORD", nil);
...
-(void)setDataForUserInfo
{
	...
	if (![[dictInfo valueForKey:@"login_by"] isEqualToString:@"manual"]) {
		[self.viewForPassword setHidden:YES];
		self.viewForPhoneNumber.frame = CGRectMake(0, self.viewForPassword.frame.origin.y, self.viewForPhoneNumber.frame.size.width, self.viewForPhoneNumber.frame.size.height);
	}
...
-(void)textFieldDidBeginEditing:(UITextField *)textField
{
    ...
    {
		if (![[[PREF objectForKey:PREF_LOGIN_OBJECT] valueForKey:@"login_by"] isEqualToString:@"manual"])
		{
			offset=CGPointMake(0, 80);
		}
		else
